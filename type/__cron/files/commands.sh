...
in
	(solaris)
		...
		;;
	(*bsd)
		crontab_print_cmd="crontab -u $(shquot "${user}") -l"
		...
		crontab_update_cmd="crontab -u $(shquot "${user}") -"
		;;
	(*)
		crontab_print_cmd="crontab -u $(shquot "${user}") -l 2>/dev/null"
		...
		_crontab_default_cmd="test -f /etc/debian_version && VISUAL= EDITOR=cat crontab -u $(shquot "${user}") -e 2>/dev/null"
